...
morse: {
    name: 'Morse code',
    help: 'Method coverts a string to/from Morse code. Use ru: at the beginning to decode into Russian.',
    intmap: [
        {r: 'AА', m: '·−'},   //ай−даа
        {r: 'BБ', m: '−···'}, //баа−ки−те−кут
        {r: 'CЦ', m: '−·−·'}, //цаа−пли−наа−ши, цаа−пли−хоо−дят
        {r: 'DД', m: '−··'},  //доо−ми−ки
        {r: 'EЕ', m: '·'},    //есть
        {r: 'FФ', m: '··−·'}, //фи−ли−моон−чик
        {r: 'GГ', m: '−−·'},  //гаа−раа−жи, гаа−гаа−рин
        {r: 'HХ', m: '····'}, //хи−ми−чи−те
        {r: 'IИ', m: '··'},   //и−ди
        {r: 'JЙ', m: '·−−−'}, //и краат−коо−ее
        {r: 'KК', m: '−·−'},  //каак же таак?, каак−де−лаа (приглашение к передаче)
        {r: 'LЛ', m: '·−··'}, //лу−наа−ти−ки
        {r: 'MМ', m: '−−'},   //маа−маа
        {r: 'NН', m: '−·'},   //ноо−мер
        {r: 'OО', m: '−−−'},  //оо−коо−лоо
        {r: 'PП', m: '·−−·'}, //пи−лаа−поо−ёт, пи−лаа−ноо−ет
        {r: 'QЩ', m: '−−·−'}, //щаа−ваам−не−шаа
        {r: 'RР', m: '·−·'},  //ре−шаа−ет
        {r: 'SС', m: '···'},  //си−ни−е, си−не−е, са−мо−лёт
        {r: 'TТ', m: '−'},    //таак
        {r: 'UУ', m: '··−'},  //у−нес−лоо
        {r: 'VЖ', m: '···−'}, //жи−ви−те−таак
        {r: 'WВ', m: '·−−'},  //ви−даа−лаа
        {r: 'XЬ', m: '−··−'}, //тоо−мяг−кий−знаак
        {r: 'YЫ', m: '−·−−'}, //ыы−не−наа−доо
        {r: 'ZЗ', m: '−−··'}, //заа−каа−ти−ки, заа−моо−чи−ки
        {r: 'Á', m: '·−−·−'},
        {r: 'ÄЯ', m: '·−·−'},  //я−маал−я−маал
        {r: 'ÉЭ', m: '··−··'}, //э−ле−ктроо−ни−ки
        {r: 'ÑЪ', m: '−−·−−'}, //твёёр−дыый−не−мяяг−киий
        {r: 'ÖЧ', m: '−−−·'},  //чее−лоо−вее−чек
        {r: 'Ш', m: '−−−−'},   //шаа−роо−ваа−рыы
        {r: 'ÜЮ', m: '··−−'},  //ю−ли−аа−наа
        {r: '1', m: '·−−−−'},  //и−тооль−коо−оо−днаа
        {r: '2', m: '··−−−'},  //две не−хоо−роо−шоо
        {r: '3', m: '···−−'},  //три те−бе−маа−лоо
        {r: '4', m: '····−'},  //че−тве−ри−те−каа
        {r: '5', m: '·····'},  //пя−ти−ле−ти−е
        {r: '6', m: '−····'},  //шеесть по−ка бе−ри
        {r: '7', m: '−−···'},  //даа−даа−се−ме−ри, сеемь сеемь хо−ро−шо
        {r: '8', m: '−−−··'},  //воо−сьмоо−гоо−и−ди
        {r: '9', m: '−−−−·'},  //ноо−наа−ноо−наа−ми
        {r: '0', m: '−−−−−'},  //нооль−тоо−оо−коо−лоо
        {r: ',', m: '−−··−−'}, //comma //_!_ руск.; гаа−даа−ли три браа−таа
        {r: '.', m: '·−·−·−'}, //period //сеть сети сеть сети
        {r: '!', m: '−·−·−−'},
        {r: '&', m: '·−···'},  //ampersand (ожидание)
        {r: '?', m: '··−−··'}, //question mark //вы ку−даа смоо−три−те?
        {r: ';', m: '−·−·−·'}, //semicolon
        {r: ',', m: '−−−···'}, //colon
        {r: '/', m: '−··−·'},  //slash //дрообь здесь пред−стаавь−те
        {r: '+', m: '·−·−·'},  //
        {r: '-', m: '−····−'}, //dash
        {r: '=', m: '−···−'},  //
        {r: "'", m: '·−−−−·'}, //apostrophe
        ...
        {r: '$', m: '···−··−'}, //
        {r: ' ', m:  '·······'}    //space
        //{r: '\r', m:  '········'}  //error|redo; хи−ми−чи−те хи−ми−чи−те
    ],
    encode: function(text) {
        var rus = /^[0-9.,-=+-;:?!@&^$"'/(){}\[\]а-яА-Я\s]+$/i.test(text);
        text = text.replace(/[\r\n\s]+/g, ' ').trim();
        for (var i = this.intmap.length; i--;)
            text = text.replace(new RegExp('['+this.intmap[i].r+']', 'ig'), this.intmap[i].m + ' ');
        return (rus ? 'ru:' : '') + text;
    },
    decode: function(text) {
        var rus = /^\s*ru:/i.test(text);
        text = text.trim().replace(/^(?:ru|en):/i, '').replace('.', '\u00b7').replace('-', '\u2212').replace(/\s+/g, '\u00a0');

        if (!this.intmap.sorted) this.intmap = this.intmap.sort(function(a, b) {
            return a.m.length - b.m.length;
        });
        this.intmap.sorted = true;

        for (var i = this.intmap.length; i--;)
            text = text.replace(new RegExp(this.intmap[i].m + '\u00a0*', 'ig'), (this.intmap[i].r[rus ? 1 : 0] || this.intmap[i].r[0]));
        return text.toLowerCase();
    },
    guess: function (text) {
        if (/^\s*(?:(?:ru|en):)?[-\u2212.\u00b7\s]+$/.test(text)) return this.decode(text);
        else return this.encode(text);
    }
}
}; /* METHODS*/
